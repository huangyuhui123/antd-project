...
export default class Permission extends Component {
    ...
    requestList = ()=>{
        axios.ajax({
            ...
            if(res.code == 0){
                let list  = res.result.list.map((item,i)=>{
                    item.key = i;
                    return item;
                })
                this.setState({
    ...
    handlePermission = ()=>{
        ...
        this.setState({
            isPermVisible: true,
            detailInfo: this.state.selectedItem  //角色信息
        });
        let menuList = this.state.selectedItem.menus; //角色对应的权限信息
        this.setState({
            menuInfo:menuList
        })
    }
    ...
    handlePermEditSubmit = ()=>{
     ...
     handleUserAuth = ()=>{
        ...
        this.getRoleUserList(this.state.selectedItem[0].id);
        this.setState({
            isUserVisible: true,
            isAuthClosed: false,
